...
const SubscriptionPage: React.FC<Props> = ({ onSuccess, onBack, t }) => {
  ...
  const finalPrice = useMemo(() => {
    return isDiscountApplied ? (basePrice * 0.95).toFixed(2) : basePrice.toFixed(2);
  }, [isDiscountApplied]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const { data: vData } = await supabase.from('vendors').select('code').ilike('code', code).maybeSingle();
        const { data: pData } = await supabase.from('profiles').select('vendor_code').ilike('vendor_code', code).maybeSingle();
        setIsDiscountApplied(!!(vData || pData));
      } catch (e) {
        setIsDiscountApplied(false);
      } finally {
    ...
    try {
      ...
      if (functionError) {
        ...
        if (realError.toLowerCase().includes('insufficient funds') || 
            realError.toLowerCase().includes('fondos insuficientes') ||
            realError.toLowerCase().includes('saldo insuficiente')) {
      ...
      if (!data?.success) {
        if (data?.error?.toLowerCase().includes('insufficient funds') || 
            data?.error?.toLowerCase().includes('fondos insuficientes') ||
      ...
      if (authData.user) {
        const { error: profileError } = await supabase.from('profiles').upsert({
          ...
          subscription: {
            ...
            isActive: true,
            appliedDiscount: wasDiscounted ? 5 : 0,
              ...
              <div className="mt-auto bg-gradient-to-br from-slate-900 via-slate-950 to-emerald-950/30 p-10 rounded-[3rem] border border-emerald-500/20 relative overflow-hidden shadow-[0_20px_50px_rgba(16,185,129,0.15)] group/card ring-1 ring-white/5">
                ...
                {isDiscountApplied && (
                  <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mt-4 animate-bounce flex items-center gap-2">
                    <Check className="w-3 h-3" /> {vendorCode === 'NEXUS-FREE-DEV' ? 'MODO TESTE ATIVO' : 'CÓDIGO APLICADO!'}
                  </p>
                )}
              </div>
            </div>
