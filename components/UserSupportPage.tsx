  ...
  useEffect(() => {
    const initializeChat = async () => {
      ...
      try {
        ...
        setErrorMessage(t('common.loading'));
      } finally {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (isHumanSupportActive) {
      ...
      try {
        ...
        setMessages(prev => [...prev, { id: 'err-' + Date.now(), role: 'ai', text: "...", timestamp: new Date() }]);
      } finally {
        setIsTyping(false);
        setIsSending(false);
        sendingRef.current = false;
  ...
  const startHumanSupport = async () => {
    ...
    try {
      ...
      const sysMsg: Message = { 
        id: 'sys-' + Date.now(), 
        role: 'support', 
        text: isOnline ? t('support.connecting') : t('support.offline'), 
        timestamp: new Date() 
      };
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20 animate-[fadeIn_0.5s_ease-out]">
      <div className="flex justify-between items-center bg-slate-900/40 p-6 rounded-[2.5rem] border border-white/5 backdrop-blur-md">
            ...
            <div className="flex items-center gap-2 mt-1">
              {isHumanSupportActive ? (
                <>
                  ...
                  <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{connectionStatus === 'online' ? t('support.agentOnline') : t('support.offline')}</span>
                </>
              ) : (
                <>
                  <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
                  <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{t('support.aiOnline')}</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex p-1 bg-slate-950/50 rounded-2xl border border-white/5">
           <button onClick={() => setView('chat')} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${view === 'chat' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-white'}`}>{t('support.agentOnline')}</button>
           <button onClick={() => setView('history')} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${view === 'history' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-white'}`}>{t('support.history')}</button>
        </div>
      </div>
      ...
      {view === 'chat' ? (
           ...
           <div className="mt-4 p-4 space-y-4">
              {!isHumanSupportActive && !isLoadingHistory && (
                <button onClick={startHumanSupport} className="w-full py-3 bg-slate-900/60 border border-blue-500/20 rounded-2xl text-[9px] font-black text-blue-400 uppercase tracking-widest hover:bg-blue-600/10 transition-all flex items-center justify-center gap-2">
                  <Headphones className="w-3.5 h-3.5" /> {t('support.humanSupportReq')}
                </button>
              )}
        ...
        <div className="bg-slate-800/20 border border-slate-800 rounded-[3rem] p-8 space-y-6 min-h-[500px]">
           <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-3">
             <History className="w-5 h-5 text-blue-400" /> {t('support.history')}
           </h3>
           <div className="grid grid-cols-1 gap-4">
              {resolvedTickets.length === 0 ? (
                <div className="py-20 text-center opacity-30">
                  <MessageSquare className="w-12 h-12 mx-auto mb-4" />
                  <p className="text-[10px] font-black uppercase tracking-widest">{t('reports.noData')}</p>
                </div>
                ...
                <div key={ticket.id} className="p-6 bg-slate-900/40 border border-white/5 rounded-2xl flex justify-between items-center group hover:border-blue-500/30 transition-all">
                   ...
                   <div className="px-4 py-1 bg-green-500/10 border border-green-500/20 rounded-full"><span className="text-[8px] font-black text-green-400 uppercase">{t('support.resolved')}</span></div>
                </div>
              ))}
           </div>
        </div>
